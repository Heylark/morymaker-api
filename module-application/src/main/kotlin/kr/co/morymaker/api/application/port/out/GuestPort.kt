...
interface GuestPort {
    ...
    /**
     * 조건부 체크인 전이(D-F, guestId 대상) — `status != '참석'`일 때만 참석으로 전이하고
     ...
     * has changed since last read")로 즉시 실패한다(REQ-0006의 `SELECT FOR UPDATE` 갭락 데드락과는
     * 다른 별개의 함정 — REQ-0019 kiosk 동시성 실 DB 검증에서 직접 재현·확인). 그래서 이 메서드는
     * "먼저 읽어 존재를 확인한 뒤 조건부로 쓴다"가 아니라 "먼저 blind 조건부 UPDATE를 시도하고,
     * 그 뒤에 상태를 다시 조회해 결과를 해석한다" 순서로 호출해야 한다([CheckinSupport] 참조).
     */
    fun markAttendedIfNotAttended(eventId: String, gid: String, visitAt: Instant): Int

    /**
     * 조건부 체크인 전이(D-F, token 대상) — [markAttendedIfNotAttended]와 동일 가드·동일 호출
