 ...
 * 게이트를 호출할 수단 자체가 없다. eventCode(=event.id, D2)의 존재·상태(D5)만으로 인가를
 * 대체한다: 무효 eventCode는 [NoSuchElementException](404), 종료 행사는
...
internal class PublicOnsiteService(
    ...
    /** eventCode(=event.id) 존재 확인 + status 게이트(D5, 종료만 거부) — 폼 조회·등록 공통. */
