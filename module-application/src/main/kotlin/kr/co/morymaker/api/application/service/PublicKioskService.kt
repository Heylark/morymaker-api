...
/**
 * [PublicKioskUseCase] 구현체(KIO-02·04·05) — 무인증 공개 경로다.
 *
 ...
 * [EventNotOpenException](409)으로 거부한다(`PublicOnsiteService.fetchOpenEvent` 원리 재사용
 * — idle-contents의 fail-open과 달리 참석자 PII를 다루므로 보수적 정책).
 *
 * 이름검색·체크인은 각각 [LookupSearchSupport]·[CheckinSupport](가드-free 코어, SCN 인증 경로와
 * 공유하는 SSOT)에 위임한다. 주차검색은 신규 SQL 없이 기존 [ParkingRecordPort.search]를
 * 직접 재사용한다(이름검색과 근본 도메인이 달라 lookup 통합 대신 전용 분기, 단 컨트롤러·서비스는
 * 단일 클래스로 응집).
 */
@Service
internal class PublicKioskService(
    ...
    override fun searchAttendees(eventId: String, name: String): LookupResult {
        ...
        // 단일 문자 대량 열거 차단 — 부분일치 자체는 코어(LookupSearchSupport) 그대로 유지.
