...
interface ParkingSlotTitleMapper {
    ...
    /** zone_id 기준 전삭제(§6-3 delete-insert 1단계). eventId는 parking_zone EXISTS 서브쿼리를 통한 cross-event 격리 방어심층(직접 컬럼 없음, ADR-3). */
    fun deleteByZoneId(@Param("eventId") eventId: String, @Param("zoneId") zoneId: String)
