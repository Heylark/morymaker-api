...
// EventScopeInterceptor(행사 스코프 게이트 Layer2a)를 /events 이하 전체 경로에 등록한다.
//
// /events·/events/{eid} 모두 이 패턴에 매칭된다(Ant 스타일 와일드카드는 0개 이상의
...
class WebMvcConfig(
    ...
    override fun addInterceptors(registry: InterceptorRegistry) {
        registry.addInterceptor(eventScopeInterceptor).addPathPatterns("/events/**")
        // 현장등록 GET(폼 렌더)·POST(등록) 모두 이 패턴에 매칭되지만, 인터셉터 내부에서 POST만
        // 실제로 검사한다(폼 조회·개인허브(/u)는 스팸 위협이 낮아 대상이 아니다).
        registry.addInterceptor(publicRateLimitInterceptor).addPathPatterns("/public/r/**")
        // 자리 QR 셀프 주차(§10-4) — GET(상태 조회)·POST(셀프 등록) 모두 매칭되지만 위와 동일하게
        // 인터셉터 내부에서 POST만 검사한다. 단측 additive(이 파일 외 공유 설정 변경 없음).
        registry.addInterceptor(publicRateLimitInterceptor).addPathPatterns("/public/p/**")
        // kiosk 개별 엔드포인트만 명시 등록(broad "/public/events/**" 아님) — idle-contents 등
        ...
        registry.addInterceptor(publicRateLimitInterceptor).addPathPatterns(
            ...
            "/public/events/*/checkin/**",
            "/public/events/*/parking-search/**",
        )
    }
}
