 ...
 *
 * 이 값들은 QR 이미지·SMS 링크·응답 JSON으로 외부에 나가는 순간 되돌릴 수 없다 — 주차 표지판은
 * 물리 재인쇄가, 초대 SMS는 회수 자체가 불가능하다(미디어 URL은 키오스크 화면에 노출되는
 * 즉시 실사용자가 소비한다). 오설정이 빌드·테스트·기동 어디에도 걸리지 않던 비대칭을 기동
 * 단계에서 막는다(issuer 가드와 같은 취지, 대상만 다르다).
 *
...
class ProdUrlConfigGuard {
    ...
    fun validate() {
        mapOf(
            ...
            "morymaker.media.base-url" to mediaBaseUrl,
